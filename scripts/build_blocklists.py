...
LISTS: dict[str, tuple[str, ...]] = {
    ...
    "combined4server": (
        "base3", "threatfox", "level4", "abuser", "botnet", "abuseipdb", "threatview"
    ),
    "complete": ("combined", "threatfox", "level2", "level3", "tif", "alienvault"),
}
...
def resolve_list_sources(
    list_name: str,
    stack: tuple[str, ...] = (),
) -> tuple[str, ...]:
    if list_name not in LISTS:
        raise RuntimeError(f"undefined list: {list_name}")

    if list_name in stack:
        cycle = " -> ".join((*stack, list_name))
        raise RuntimeError(f"LISTS contains a cycle: {cycle}")

    resolved: list[str] = []
    seen: set[str] = set()
    next_stack = (*stack, list_name)

    for name in LISTS[list_name]:
        if name in SOURCES:
            source_names = (name,)
        elif name in LISTS:
            source_names = resolve_list_sources(name, next_stack)
        else:
            raise RuntimeError(
                f"{list_name}: undefined source or list: {name}"
            )
...
def required_sources() -> tuple[str, ...]:
    ...
    for list_name in LISTS:
        for source_name in resolve_list_sources(list_name):
            if source_name not in seen:
                seen.add(source_name)
...
def validate_config() -> None:
    ...
    for output_name, source_names in LISTS.items():
        ...
        if not source_names:
            raise RuntimeError(
                f"{output_name}: at least one source or list is required"
            )
        if len(source_names) != len(set(source_names)):
            raise RuntimeError(
                f"{output_name}: duplicate source/list references are not allowed"
            )

        # Resolve now so unknown references and cycles fail before downloads start.
        resolved = resolve_list_sources(output_name)
        if not resolved:
            raise RuntimeError(
                f"{output_name}: resolves to no sources"
            )
...
def fetch_source(name: str) -> set[ipaddress.IPv4Network]:
    ...
    for line_number, raw_line in enumerate(body.splitlines(), 1):
        ...
        if not found:
            ignored_lines += 1
            print(
                f"warning: {name}: ignored line without valid IPv4/CIDR "
    ...
    if ignored_lines:
        print(
            f"warning: {name}: ignored {ignored_lines:,} non-IP line(s)",
            file=sys.stderr,
        )
...
def build_lists(
    ...
    for configured_name in LISTS:
        output_name = safe_name(configured_name)
        source_names = resolve_list_sources(configured_name)
